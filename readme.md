# Macsim (Revived)
## Introduction
This documentation explains how you can run various GPU workloads on Macsim, a trace based cycle-level GPGPU simulator.
...
```
Note that you need three input files: `macsim` (binary executable), `params.in` (GPU configuration), and `trace_file_list` (list of paths to GPU traces). All these three files should be in the same directory to run Macsim.

### Running
#### 1. Setup the Trace Path

Open trace_file_list and leave 1 on the first line. On the second line, change it to the path to the trace you want to run. An example of trace_file_list would look like this:
```
1
